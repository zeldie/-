...
CFoot::CFoot(LPDIRECT3DDEVICE9 pGraphicDev)
...
HRESULT CFoot::Change_Foot(_uint iFoot, _uint iCurrentTrack, _double dTrackTime, _uint iAnimationNum)
{
...
HRESULT CFoot::Ready_GameObject(_vec3 * pPos, _vec3 * pAngle, _vec3 * pScale, _uint iFootNum)
{
	Load_Foot();
	if (FAILED(Clone_Component(iFootNum)))
		return E_FAIL;
	...
	m_iFootIndex = iFootNum;
	return S_OK;
}

_int CFoot::Update_GameObject(const _double & dTimeDelta, _vec3 * pPos, _vec3 * pAngle, _vec3 * pScale, _uint iAnimationNum)
{
	m_pTransformCom->Set_Info(pPos, Engine::INFO_POS);
	m_pTransformCom->Set_Angle(pAngle);
	m_pTransformCom->Set_Scale(pScale);
...
_int CFoot::LateUpdate_GameObject(const _double & dTimeDelta)
{
...
void CFoot::Render_Geometry(const _double & dTimeDelta)
{
...
void CFoot::Render_PostEffect(const _double & dTimeDelta)
{
	////Shader
	//LPD3DXEFFECT	pEffect = m_pShaderCom->Get_EffectHandle();
	//if (pEffect == nullptr)
...
//void CFoot::Render_Shadow(const _double & dTimeDelta)
//{
//	////Shader
//	//LPD3DXEFFECT	pEffect = m_pShaderCom->Get_EffectHandle();
//	//if (pEffect == nullptr)
...
HRESULT CFoot::Clone_Component(_uint iFootNum)
{
	...
	pComponent = m_pDynamicMeshCom = m_vecFoot[iFootNum];
	if (pComponent == nullptr)
		return E_FAIL;
...
HRESULT CFoot::Setup_ShaderProps(LPD3DXEFFECT & pEffect)
{
	CBaseObject::Set_ShaderMatrix(pEffect);
	return S_OK;
}

HRESULT CFoot::Load_Foot()
{
	wstring wstrFootOriID = L"Mesh_Foot";
	Engine::CDynamicMesh* pDynamicMesh = nullptr;
	for (_uint i = 0; i < m_iFootNum; ++i)
	{
		wstring wstrFootID = wstrFootOriID + to_wstring(i);
		pDynamicMesh = static_cast<Engine::CDynamicMesh*>(Engine::Clone(Engine::RESOURCE_STATIC, wstrFootID));
		m_vecFoot.emplace_back(pDynamicMesh);
	}
	return S_OK;
}

CFoot * CFoot::Create(LPDIRECT3DDEVICE9 pGraphicDev, _vec3 * pPos, _vec3 * pAngle, _vec3 * pScale, _uint iFootNum)
{
	CFoot* pInstance = new CFoot(pGraphicDev);
	if (FAILED(pInstance->Ready_GameObject(pPos, pAngle, pScale, iFootNum)))
...
void CFoot::Free()
{
	for (_uint i = 0; i < m_iFootNum; ++i)
	{
		if (i == m_iFootIndex)
			continue;
		Engine::Safe_Release(m_vecFoot[i]);
	}
	m_vecFoot.clear();
