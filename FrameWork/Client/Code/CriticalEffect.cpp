...
CCriticalEffect::CCriticalEffect(LPDIRECT3DDEVICE9 pGraphicDev)
...
CCriticalEffect::~CCriticalEffect()
{
}

HRESULT CCriticalEffect::Ready_GameObject(_vec3* pPos, _vec3* pAngle, BASE_INFO* pBaseInfo)
{
...
_int CCriticalEffect::Update_GameObject(const _double & dTimeDelta)
{
	if (m_bIsDead)
...
_int CCriticalEffect::LateUpdate_GameObject(const _double & dTimeDelta)
{
...
void CCriticalEffect::Render_Geometry(const _double & dTimeDelta)
{
...
void CCriticalEffect::Set_Info(_vec3* pPos, _vec3* pAngle, BASE_INFO* pBaseInfo)
{
	ResetTextureInfo();
	ResetEffectInfo();
...
HRESULT CCriticalEffect::Clone_Component()
{
	...
	//Texture
	pComponent = m_pTextureCom = dynamic_cast<Engine::CTexture*>(Engine::Clone(Engine::RESOURCE_STATIC, L"Texture_Critical"));
	if (pComponent == nullptr)
		return E_FAIL;
...
HRESULT CCriticalEffect::Setup_ShaderProps(LPD3DXEFFECT & pEffect)
{
...
void CCriticalEffect::Transform(const _double& dTimeDelta)
{
...
void CCriticalEffect::ResetTextureInfo()
{
...
CCriticalEffect * CCriticalEffect::Create(LPDIRECT3DDEVICE9 pGraphicDev, _vec3 * pPos, _vec3 * pAngle, BASE_INFO * pBaseInfo)
{
	CCriticalEffect* pInstance = new CCriticalEffect(pGraphicDev);
