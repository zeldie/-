                     ...
                     _In_ int       nCmdShow)
{
	...
	// Frame
	if (FAILED(Engine::Ready_Frame(L"Frame60", 60)))
		return E_FAIL;
	...
	while (true)
	{
		...
		else
		{
			...
			if (Engine::IsPermit_Call(L"Frame60", dTimeDelta))
			{
				...
				EVENT_CHECK(pMainApp->Update_MainApp(dTimeDelta60));
				EVENT_CHECK(pMainApp->LateUpdate_MainApp(dTimeDelta60));
				pMainApp->Render_MainApp(dTimeDelta60);
			}
			//EVENT_CHECK(pMainApp->Update_MainApp(dTimeDelta));
			//EVENT_CHECK(pMainApp->LateUpdate_MainApp(dTimeDelta));
			//pMainApp->Render_MainApp(dTimeDelta);
		}
